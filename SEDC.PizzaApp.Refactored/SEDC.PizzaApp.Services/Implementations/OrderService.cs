...
namespace SEDC.PizzaApp.Services.Implementations
{
    public class OrderService : IOrderService
    {
        ...
        public List<PizzaDDViewModel> GetPizzasFromOrder(int id)
        {
            Order order = _orderRepository.GetById(id);
            List<PizzaDDViewModel> pizzasFromOrder = new List<PizzaDDViewModel>();
            foreach (var item in order.PizzaOrders)
            {
                PizzaDDViewModel pizza = new PizzaDDViewModel
                {
                    Id = item.PizzaId,
                    Name = item.Pizza.Name
                };
                pizzasFromOrder.Add(pizza);
            }
